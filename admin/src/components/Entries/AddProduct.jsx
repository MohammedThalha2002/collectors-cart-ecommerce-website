...
function AddProduct({ update, onBack }) {
  // image upload
  ...
  const [dealUpdate, setDealUpdate] = useState(false);
  ...
  function handleSubmit(e) {
    ...
    if (contentType == "Deal of the day") {
      if (dealUpdate) {
        state = `/updateDealOftheDay/${1}`;
      } else {
        state = "/addDealOftheDay";
      }
    }

    // console.log(state);

    axios.post(import.meta.env.VITE_API_URL + state, data).then((res) => {
      // console.log(res.data);
      if (update) {
        showToast(toast, "success", "Product Updated successfully");
      } else {
        showToast(toast, "success", "Product Uploaded successfully");
      }
      if (contentType !== "Deal of the day") onBack();
    });
  ...
  useEffect(() => {
    if (contentType == "Deal of the day") {
      ...
      axios
        .get(import.meta.env.VITE_API_URL + `/dealOftheDay/1`)
        .then((res) => {
          // console.log(res.data);
          const data = res.data;
          if (data) {
            setDealUpdate(true);
            // console.log("UPDATE DEAL OF THE DAY");
            setName(data.Name);
            setDescription(data.Description);
            setImage(data.Image);
            setSelectedImage(import.meta.env.VITE_IMAGE_URL + data.Image);
            ...
            // console.log("ADD NEW DEAL OF THE DAY");
          }
        });
      // .catch((err) => console.log(err));
    }
          ...
          <div className="flex gap-4">
            ...
            <div className="basis-[50%]">
              ...
              <img
                ...
                alt=""
                className="hover:cursor-pointer rounded-md h-72 w-72 object-cover"
                id="image"
